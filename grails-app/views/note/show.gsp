	...
	<body>
		...
		<div id="show-note" class="content scaffold-show" role="main">
			...
			<ol class="property-list note">
				...
				<g:if test="${noteInstance?.pointInteret}">
				<li class="fieldcontain">
					<span id="pointInteret-label" class="property-label"><g:message code="note.pointInteret.label" default="Point Interet" /></span>
					
						<span class="property-value" aria-labelledby="pointInteret-label"><g:link controller="POI" action="show" id="${noteInstance?.pointInteret?.id}">${noteInstance?.pointInteret?.encodeAsHTML()}</g:link></span>
					
				</li>
				</g:if>
			
				<g:if test="${noteInstance?.utilisateur}">
				<li class="fieldcontain">
					<span id="utilisateur-label" class="property-label"><g:message code="note.utilisateur.label" default="Utilisateur" /></span>
					
						<span class="property-value" aria-labelledby="utilisateur-label"><g:link controller="utilisateur" action="show" id="${noteInstance?.utilisateur?.id}">${noteInstance?.utilisateur?.encodeAsHTML()}</g:link></span>
